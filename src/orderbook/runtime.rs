use std::{
    ...
    fmt::Debug,
    path::Path, time::{SystemTime, Duration}, str::FromStr
};
...
use crate::{MessageEnum, callback_datatype};
...
where
    A: OrderBookRunTimeCallback + Clone
{
    let mut runtime = OrderBookRunTime::new(callback.clone());
    runtime.order_book_map = order_book_map.clone();
    // list of all order book id who had something changes to price levels
    while let Some((timestamp, stack)) = key_as_timestamp.next() {
        runtime.process_timeframe(timestamp, stack);
    }
    runtime.all_done();
    runtime
}

pub struct OrderBookRunTime<A: OrderBookRunTimeCallback> {
    order_book_map: HashMap<u64, OrderBook>,
    callback: A,
...
impl<A> OrderBookRunTime<A>
    where
        A: OrderBookRunTimeCallback
{
    pub fn new(callback: A) -> Self {
        Self { order_book_map: HashMap::new(), callback, last_timestamp: None, message_count: 0, key_count: 0, now: SystemTime::now() }
    }
    pub fn all_done(&mut self) {
        self.callback.all_done(&mut self.order_book_map, self.last_timestamp);
        let stats = RuntimeStats {
            message_count: self.message_count,
            key_count: self.key_count,
            time_taken: self.now.elapsed().unwrap()
        };
        println!("Done!\nmessage count: {}\nkey count: {}\ntime taken: {:?}", stats.message_count, stats.key_count, stats.time_taken);
    }
    /// this function is company and pased
    pub fn process_timeframe(&mut self, timestamp: NaiveDateTime, stack: Vec<MessageEnum>) -> bool {
        let callback = &mut self.callback;
        let order_book_map = &mut self.order_book_map;
        let mut changes = HashSet::new();
        
            if callback.stop() {
                return false;
            }
            self.last_timestamp.replace(timestamp);
            // sort stack
